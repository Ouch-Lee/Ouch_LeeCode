...
层序遍历比较直观，就是一层一层的遍历下去，因为符合排队的思维，所以可以使用一个队列结构，把最前面的节点放进来，然后取出，再把它的两个子节点放进来，一直取到队列空的，就把树遍历完了；需要注意的是【每一层开始时，需要**计算队列的尺寸**，然后后续无论是添加到数组还是队列添加左右子节点，都只需要执行size次，】
...
* [从前序与中序遍历序列构造二叉树](https://leetcode.cn/problems/construct-binary-tree-from-preorder-and-inorder-traversal/)，[笔记如下](#前序和中序遍历中构造二叉树)
* [前序和中序遍历中构造二叉树](#中后序遍历构造二叉树)
...
递归问题适合解决需要一层一层走进去，然后最后又要走回来的问题，走进去就是所谓的递，走回来就是归，但是什么时候回来，就是递归的尽头；另外思考递归时，不要关注二叉树的细节，而是把左、右子树看作一个整体，这样就只用关心相邻层之间的关系；
